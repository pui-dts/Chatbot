     ...
     //context.Wait(this.Translate);
{
    ...
    public class ThaiDialog : IDialog<string>
    {
        ...
        private async Task MessageQnA(IDialogContext context, IAwaitable<IMessageActivity> result)
        {
            ...
            PromptDialog.Choice(context, this.OnSolveSelect, yesNoOptions, ans2, "กรุณาเลือกอีกครั้ง", 3);
            ...
            try
            {
                ...
                switch (optionSelected)
                {
                    ...
                    case "ใช่":
                        ...
                        actions.Add(new CardAction { Type = ActionTypes.ImBack, Title = $"{ this.user.ImageRate1Url1 }", Value = $"2" });
                        actions.Add(new CardAction { Type = ActionTypes.ImBack, Title = $"{ this.user.ImageRate1Url1 }", Value = $"3" });
                        actions.Add(new CardAction { Type = ActionTypes.ImBack, Title = $"{ this.user.ImageRate1Url1 }", Value = $"4" });
                        actions.Add(new CardAction { Type = ActionTypes.ImBack, Title = $"{ this.user.ImageRate1Url1 }", Value = $"5" });
